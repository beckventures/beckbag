...
var paymentlist = [ 'Pending', 'Processing', 'Completed' ];
...
class CompletedPackages extends React.Component {
    ...
    database.ref('beckbag').child('merchantorders').child('matched')
        ...
        .then((snapshot) => {
          ...
          snapshot.forEach((chSnapshot) => {
            ...
            packaged.payment = paymentlist[index];
            packaged.key = packaged.orderid;
            packaged.value = '₹  ' + packaged.value;
            var fee = packaged.weight * packaged.basecharge;
            console.log(fee);
            packaged.fee = '₹  ' + fee;
          ...
          <Row>
            <Col md="12">
              <Card className="customcard" style={{ backgroundColor: 'transparent' }}>
                <CardHeader>
                  <CardTitle tag="h4" style={{ fontSize: '1.5em', fontWeight: '800', marginBottom: '25px' }}>List Of Completed Packages</CardTitle>
                </CardHeader>
                <CardBody>
