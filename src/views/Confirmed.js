...
import ListGroupCollapseConfirmed from './ListGroupCollapseConfirmed';
...
class Confirmed extends React.Component {
  ...
  const that = this;
   ...
   database.ref('beckbag').child('carrier').child('airindia').child('trips').child('pendingflights')
        .once('value')
        .then((snapshot) => {
          ...
          snapshot.forEach((chSnapshot) => {
            ...
            chSnapshot.forEach((chchSnapshot) => {
              ...
              database.ref('beckbag').child('carriertrips').child('added').child(tripid)
              .once('value')
              .then((snapshotpackage) => { 
                count++;
                var weight = snapshotpackage.val().weight;
          ...
          <Row>
            ...
            <Card className="customcard" style={{ backgroundColor: 'transparent', boxShadow: 'none !important' }}>
                <CardHeader style={{ display: 'flex', justifyContent: 'center' }}>
                  <CardTitle tag="h4" style={{ fontSize: '1.5em', fontWeight: '800', textAlign: 'center' }}>Pending Trips</CardTitle>
              ...
              <ListGroupCollapseConfirmed key={airline} cat={airline} ball={airline} />
              )}
              </Container>}
              </CardBody>
              </Card>
